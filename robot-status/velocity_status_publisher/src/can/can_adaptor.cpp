...
int CanAdaptor::ROpen(vector<string> device){
  ...
  try{
    ...
    recvthread.detach();
    */
    std::shared_ptr<CanDump> can_dump = std::make_shared<CanDump>();
    can_dump->Open(parametermap.size(), argval,this,&CanAdaptor::Receive);
    //recvthread.join();
...
void CanAdaptor::Receive(byte* data,int canid) {
  std::cout<<__LINE__<<' '<<canid<<' '<<funcsmap_.size()<<'\n';
    if(funcsmap_.size()>0){
