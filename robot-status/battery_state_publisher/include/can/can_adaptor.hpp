...
class CanAdaptor {
  ...
  private:  
    ...
    typedef std::function<void(BMS_System_Info)> func_BMS_System_Info;


    func_BMS_System_Info handler_bsi;
    bool isBigEndian_ = 0;
  ...
  public:   
    ...
    */
    ...
    template<typename T>
    void SetHandler(T *pClassType,void(T::*pfunc)(BMS_System_Info),int canid,string device){        
      handler_bsi = move(bind(pfunc, pClassType, placeholders::_1));
     // int canid = string2hex(msgid);
      ...
      std::shared_ptr<CanCallbackFunc> object = std::make_shared<CanCallbackFunc>(
                ...
                ,[&](byte* data) { 
                  ...
                  handler_bsi((BMS_System_Info)r);
                  //cout<< "end handler_mtf" << endl;
                }
                );
      
       cout << "setHandler(BMS_System_Info) : " + device << ", canid : "<< canid << endl;          
